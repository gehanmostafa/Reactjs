              ...
              {DataImage.map((item) => (
                ...
                >
                  <Link to={`/gallery/${item.category}`}>
                    <div className="w-[100%]  h-full rounded-[50%]   relative group cursor-pointer ">
                      <img
                        src={item.imgUrl}
                        alt={`Image ${item.id}`}
                      ...
                      <div className="absolute inset-0 flex justify-center text-center    items-center w-[70%] rounded-[50%]  group-hover:bg-[rgba(255,255,255,0.6)] transition-colors duration-150">
                        <div>
                          <span className="relative font-bold opacity-0  group-hover:opacity-100 text-black text-[40px] transition-opacity duration-700 ease-in-out">
                            {item.category}
                          </span>
                        </div>
                      </div>
